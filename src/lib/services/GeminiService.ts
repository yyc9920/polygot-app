...
export const GeminiService = {
  async generatePhrases(context: string, count: number, apiKey: string): Promise<GeneratedPhrase[]> {
    ...
    return JSON.parse(resultText) as GeneratedPhrase[];
  },

  async generateSongLyrics(artist: string, title: string, apiKey: string, targetLanguage: string, locale: string) {
  ...
  async generatePhraseFromLyric(lyric: string, translated: string, artist: string, title: string, apiKey: string, locale: string) {
  ...
  async explainPhrase(sentence: string, meaning: string, pronStr: string, tagsStr: string, targetLangName: string, apiKey: string) {
    const prompt = `
